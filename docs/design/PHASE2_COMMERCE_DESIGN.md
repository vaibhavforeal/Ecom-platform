...
| D17 | Settlement economics | **Capture `fee_paise` / `fee_tax_paise` from the webhook payload onto the payments row**; order detail renders net = gross − fee, with the fee GST shown as a parenthetical component INSIDE the fee (e.g. "Fee ₹23.60 (incl. ₹3.60 GST)") | Judge 1 graft 7 / judge 2 graft 6: blueprint §5.2 asks for it; the data is free at the webhook writer. Razorpay's `fee` field already INCLUDES GST — `tax` is the component inside it — so subtracting `fee_tax_paise` a second time would understate settlement by the GST on every payment |
...
**apps/console**
- `src/app/(dashboard)/orders/page.tsx` + `orders/[id]/page.tsx` [NEW] [B5] — list (status filter, COD-due column) and detail: lines, payments **with net-settlement line (gross − fee; the fee already includes its GST, D17)**, order_events timeline, **action buttons rendered ONLY from ORDER_TRANSITIONS' legal next states (D12)**, cancel dialog.
